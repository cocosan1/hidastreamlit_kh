...
#************************ファイルのdf化・加工
# ***今期出荷***
path_snow = os.path.join(cwd, 'data', 'kita81s.xlsx')
...
#目標
target_list_kh = [5900000, 6200000, 5500000, 6500000, 5700000, 6300000,
                  6600000, 6300000, 6800000, 8100000, 6200000, 6000000] 

target_list_w = [7350000, 8850000, 8450000, 8050000, 7850000, 8650000, \
          8650000, 8650000, 8850000, 8650000, 8350000, 8350000] 


def make_df(employee_code, df_snow, df_jnow, df_jlast, df_jlast_full, target_list):
    df_snow = df_snow[df_snow['営業担当コード']==employee_code]
    df_jnow = df_jnow[df_jnow['営業担当コード']==employee_code]
    df_jlast = df_jlast[df_jlast['営業担当コード']==employee_code]
    df_jlast_full = df_jlast_full[df_jlast_full['営業担当コード']==employee_code]
...
for t_list,  employee_code in zip([target_list_kh, target_list_w], [952, 854]):

    st.write(employee_code)
    # 担当毎にdfの作成
    df_month2 = make_df(employee_code, df_snow, df_jnow, df_jlast, df_jlast_full, t_list)
    # チャートの作成
    make_chart1(df_month2)
    
# #***********************************累計　出荷ベース可視化

def make_chart2(df_month2s):
...
for t_list,  employee_code in zip([target_list_kh, target_list_w], [952, 854]):
    ...
    df_month2 = make_df(employee_code, df_snow, df_jnow, df_jlast, df_jlast_full, t_list)
    # チャートの作成
    make_chart2(df_month2)

# #*****受注ベース可視化

def make_chart3(df_month2):
    #グラフ用にint化
    ...
    fig3 = go.Figure()
    #今期のグラフの追加
    for col in df_month2.columns[10:13]:
        fig3.add_trace(
            go.Scatter(
                x=['10月', '11月', '12月', '1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月'], #strにしないと順番が崩れる
                y=df_month2[col],
...
for t_list,  employee_code in zip([target_list_kh, target_list_w], [952, 854]):
    ...
    # チャートの作成
    make_chart3(df_month2)

# #*****累計 受注ベース可視化

def make_chart4(df_month2j):
    #グラフ用にint化
    df_month2['受注/今期2'] = df_month2['受注/今期'].apply(lambda x: x.replace(',', '')).astype('int')
    df_month2['受注/前期2'] = df_month2['受注/前期'].apply(lambda x: x.replace(',', '')).astype('int')
    df_month2['受注/前期年間2'] = df_month2['受注/前期年間'].apply(lambda x: x.replace(',', '')).astype('int')
    ...
    fig4 = go.Figure()
    #今期のグラフの追加
    for col in df_month2j.columns[13:16]:
        fig4.add_trace(
            go.Scatter(
                x=['10月', '11月', '12月', '1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月'], #strにしないと順番が崩れる
                y=df_month2j[col],
